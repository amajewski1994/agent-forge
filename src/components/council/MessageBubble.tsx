...
function ConflictEventCard({ message }: MessageBubbleProps) {
  return (
    <div className="animate-slide-up rounded-xl bg-amber-950/20 border border-amber-500/25 px-3.5 py-2.5">
      <div className="flex items-center gap-2 mb-1">
        ...
        <span className="text-[10px] font-bold text-amber-300 uppercase tracking-wider">Conflict Detected</span>
        <span className="text-[10px] text-slate-700">{message.timestamp}</span>
      </div>
      <p className="text-xs font-semibold text-amber-200 mb-0.5">{message.conflictTitle}</p>
      <p className="text-[11px] text-slate-500 leading-relaxed">{message.content}</p>
    </div>
  );
}
    ...
    <div className="animate-slide-up rounded-xl bg-indigo-950/20 border border-indigo-500/20 overflow-hidden">
      <div className="flex items-center justify-between gap-2 px-3.5 pt-2.5 pb-2">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] font-bold text-indigo-300 uppercase tracking-wider">Vote Complete</span>
        </div>
        <span className="text-[10px] text-slate-700">{message.timestamp}</span>
      </div>

      <div className="px-3.5 pb-2.5 border-t border-indigo-500/10">
        <p className="text-[10px] text-slate-600 mt-1.5 mb-0.5">{conflictTitle}</p>
            ...
            {votes.map((vote, i) => {
              ...
              return (
                <div key={i} className="flex gap-2">
                  <span className={`text-[10px] font-bold w-7 shrink-0 ${agentColor}`}>
                    {vote.agentAbbr}
                  </span>
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[10px] text-slate-600 leading-relaxed">{vote.reason}</p>
                  </div>
                </div>
              );
            })}
...
export default function MessageBubble({ message }: MessageBubbleProps) {
  if (message.type === "conflict") return <ConflictEventCard message={message} />;
  if (message.type === "conflict_result") return <ConflictResultCard message={message} />;
  ...
  return (
    <div className="flex gap-3 animate-slide-up">
      ...
      <div className="flex-1 min-w-0">
        ...
        <div
          className={[
            "rounded-xl rounded-tl-sm px-3.5 py-2.5 text-sm leading-relaxed whitespace-pre-line",
