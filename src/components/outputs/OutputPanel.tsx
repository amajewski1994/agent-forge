    ...
    <aside className="flex flex-col overflow-hidden h-full">
      ...
      <div className="flex-1 overflow-y-auto scrollbar-hide p-3 space-y-5">
        ...
        {topicSummaries.length > 0 && (
          <section>
            <SectionHeader title="Stages" count={topicSummaries.length} />
        ...
        {(phase === "output" || phase === "complete") && (
          <div className="animate-slide-up">
            {isPrdGenerating ? (
              <button
                disabled
                className="w-full flex items-center justify-center gap-2 rounded-xl px-3 py-2.5 bg-slate-800 border border-slate-700/60 text-slate-400 text-xs font-semibold cursor-not-allowed"
              >
                <svg className="w-3.5 h-3.5 animate-spin" fill="none" viewBox="0 0 24 24">
                  <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
                  <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8H4z" />
                </svg>
                Generating MVP Package…
              </button>
            ) : prd ? (
              <button
