...
const { runTechnicalArchitectureFlow } = require("./flows/technicalArchitectureFlow");
const {
  AGENT_META,
...
async function runDynamicDiscussion({ idea, topic, messages, topicStartIndex, resolvedTopics, resolvedDecisions, sendMessage, send }) {
  ...
  sendMessage({ agentAbbr: firstResponderMeta.agentAbbr, role: firstResponderMeta.role, content: firstResponse, type: "message" });
  ...
  for (let i = 0; i < MAX_REPLIES_PER_TOPIC; i++) {
    ...
    const candidateAgents = Object.keys(AGENT_META).filter(
      (key) => AGENT_META[key].agentAbbr !== lastMessage.agentAbbr && responseCount[key] < MAX_RESPONSES_PER_AGENT,
    );

    const evaluations = [];
    for (const agentKey of candidateAgents) {
      const evaluation = await evaluateAgentInterest({ agentKey, idea, lastMessage, conversationHistory, resolvedDecisions, topic });
      evaluations.push({ agentKey, ...evaluation });
    }

    evaluations.sort((a, b) => b.relevance - a.relevance);
    const winner = evaluations[0];
    if (!winner || (i >= MIN_REPLIES_PER_TOPIC && winner.relevance < MIN_RELEVANCE)) break;

    const response = await generateCheckedAgentReply({ agentKey: winner.agentKey, idea, conversationHistory, lastMessage, resolvedDecisions, topic });
  ...
  if (conflict.hasConflict) {
    ...
    sendMessage({ agentAbbr: "PM", role: "Product Manager", content: `Mamy różnicę zdań w kwestii "${conflict.topic}". Przechodzimy do głosowania.`, type: "message" });
    ...
    sendMessage({ agentAbbr: "PM", role: "Product Manager", content: "Widzę że jesteśmy zgodni. Oddaję głos CEO w celu podjęcia decyzji.", type: "message" });
  }
...
async function buildCouncilWorkflow(idea, options = {}) {
  ...
  console.log("[Project Classification]", JSON.stringify(projectCategory, null, 2));
  ...
  const topicsToRun = agenda.slice(7, 8);
  ...
  for (let topicIndex = 0; topicIndex < topicsToRun.length; topicIndex++) {
    ...
    if (stageNumber > 1) {
      sendMessage({ agentAbbr: "PM", role: "Product Manager", content: `Przechodzimy do punktu numer ${stageNumber}: ${topic.title}.`, type: "message" });
    }
    ...
    if (topic.title === "Product Vision") {
      ...
      ({ ceoDecision } = await runTechnicalArchitectureFlow({ idea, topic, resolvedDecisions, messages, topicStartIndex, sendMessage, send }));
    } else {
    ...
    sendMessage({ agentAbbr: "CEO", role: "Decision Leader", content: ceoDecision, type: "decision" });
    resolvedDecisions.push({ topic: topic.title, winner: ceoDecision });

    const summaryText = customStageSummary ?? await generateTopicSummary({
    ...
    if (!isLastTopic && waitForProceed) {
      ...
      sendMessage({ agentAbbr: "PM", role: "Product Manager", content: `Zamknęliśmy temat "${topic.title}". Czy możemy przejść do kolejnego punktu: "${nextTopic.title}"?`, type: "message" });
  ...
  sendMessage({ agentAbbr: "PM", role: "Product Manager", content: pmClosing, type: "message" });
